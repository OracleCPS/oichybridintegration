...
- Select the *Enable tracing*, then *Include payload* checkboxes since this isn’t a production deployment.  These checkboxs will tell ICS that the payloads for each instance of the integration will be saved in the logfiles as explored in the first part of this lab earlier.
...
- A message will appear on the top of the ICS Designer Portal indicating that the integration activation was successful.  The link for the service endpoint will also be displayed.

- Click on the link so we can ensure that this integration has an available service endpoint.

	![](images/300/lab3032.png)

- The Endpoint Description for you new ICS integration will now be displayed in your browser.
...
- In your Workspace, a new tab should open. If not, Click on the **New** button so we can create a new GET request for testing our new ICS REST Web Service.  
...
You have now completed Lab 300 of the ICS Developer Workshop.